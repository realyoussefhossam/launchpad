...
contract DutchAuctionLaunchPad is BaseHook {
        ...
        address _paymentToken;
        {
            (
                ...
                address __liquidityInitializer
                    ...
                    data,
                    (int256, int256, int256, int256, int256, address, address)
                );
            if (
                _salePeriod < 0 ||
            ...
            lastAvailableAuctionStartTime[poolId] = sd(
                int256(block.timestamp) * 1e18
            );
    ...
    function beforeSwap(
        ...
        if (
            sd(int256(block.timestamp) * 1e18) >
            lastAvailableAuctionStartTime[poolId] + salePeriod[poolId]
        ) {
        ...
        if (auctionOver[poolId]) {
            ...
            SD59x18 secondsOfEmissionsAvailable = sd(
                int256(block.timestamp) * 1e18
            ) - lastAvailableAuctionStartTime[poolId];
            SD59x18 secondsOfEmissionsToPurchase = sd(
                int256(params.amountSpecified) * 1e18
            ).div(emissionRate[poolId]);
            ...
            }
            {
                ...
                SD59x18 timeSinceLastAuctionStart = sd(
                    int256(block.timestamp) * 1e18
                ) - lastAvailableAuctionStartTime[copyPoolId];
                SD59x18 num1 = initialPrice[copyPoolId].div(
                    decayConstant[copyPoolId]
                );
                SD59x18 num2 = decayConstant[copyPoolId]
                    .mul(quantity)
                    .div(emissionRate[copyPoolId])
                    .exp() - sd(1e18);
                SD59x18 den = decayConstant[copyPoolId]
            ...
            Currency.wrap(paymentToken[poolId]).take(
                manager,
                address(this),
                unspecifiedAmount,
                true
            );
            Currency.wrap(tokenToSell[poolId]).settle(
                manager,
                address(this),
