...
contract DutchAuctionLaunchPadTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            address(this),
            flags,
            type(DutchAuctionLaunchPad).creationCode,
            abi.encode(address(manager))
        );
        hook = new DutchAuctionLaunchPad{salt: salt}(
            IPoolManager(address(manager))
        );
        require(
            hookAddress == address(hook),
            "Dutch Auction Launch Pad: hook address mismatch"
        );
        // Create the pool
        ...
        IERC20(Currency.unwrap(Deployers.currency1)).approve(
            address(hook),
            3_000_000
        );
    ...
    function unlockCallback(
        ...
        require(
            ...
            "Dutch Auction Launch Pad: unlockCallback sender is not the manager"
        );
        (PoolKey memory key, uint160 sqrtPriceX96, bytes memory hookData) = abi
    ...
    function testSwapWithoutPrank() public {
        ...
        assertEq(
            balanceCurrency0BeforeSwapping - balanceCurrency0AfterSwapping,
            66925,
            "Incorrect amount of currency1 swapped"
        );
        assertEq(
            balanceCurrency1AfterSwapping - balanceCurrency1BeforeSwapping,
            5,
            "Incorrect amount of currency0 swapped"
        );
    }
    ...
    function testSwapWithPrank() public {
        ...
        skip(10);
        uint256 balanceCurrency0BeforeSwapping = currency0.balanceOf(
            address(1)
        );
        uint256 balanceCurrency1BeforeSwapping = currency1.balanceOf(
            address(1)
        );
        vm.startPrank(address(1));
        IERC20(Currency.unwrap(Deployers.currency0)).approve(
            address(swapRouter),
            1_000_000
        );
        swap(poolKey, true, 5, abi.encode(address(this)));
